...
Input is growth data from 1 bioscreen runs, containing 2-3 biological replicates and 3 technical reps per biorep, of OD600 data of, hstA KO (Hv253), and parent WT (Hv28) in complex but less rich media (HvCa).
Colour scheme throughout: blue=Hv28=parent=WT; red=Hv253=hstA=KO. 
Since area under the curve is being compared, I took care to cut off the data early into stationary phase (timestop=65 hrs) in order to not bias the results with a slightly different final OD in stat phase.
...
Growthtable=read_excel("HvCa_hstA_J21_growth.xlsx")
...
for (i in (1:l3)) {
  ...
  plot=ggplot(melted,aes(x=Time,y=OD,colour=Strain))+scale_colour_manual(values=c("red","cadetblue3"))+scale_fill_manual(values=c("red","cadetblue3"))+geom_smooth(se=TRUE,level=0.99,aes(col=Strain,fill=Strain))+theme_bw()+geom_line(size=0.1,alpha=0.35,aes(group=variable))+xlim(c(0,timestop))+scale_y_log10()+ggtitle(paste(Media[i],"OD vs time - semi-log plot",sep=" "))+xlab("Time (hrs.)")+ylab("OD600")+theme(axis.text=element_text(family="sans",size=12,face="bold"),axis.title=element_text(family="sans",size=14,face="bold"))
  print(plot)
}
...
for (i in (1:l3)) {
  ...
  plot=ggplot(output,aes(x=fct_rev(AddId),y=integral.model))+geom_jitter(width=0.2,aes(color=AddId))+stat_summary(fun=median,geom="crossbar",alpha=0.3,width=0.5,aes(color=AddId))+scale_color_manual(values=c("red","cadetblue3"))+geom_signif(data=output,comparisons = list(c("hstA","WT")),test="t.test",map_signif_level=TRUE)+ggtitle(paste(Media[i],"Area under semi-log curve vs genotype",sep=" ")) + theme_bw()+xlab("Strain")+ylab("Area under the curve") + theme(axis.text=element_text(family="sans",size=12,face="bold"),axis.title=element_text(family="sans",size=14,face="bold"))
  print(plot)
}
```
Export AUC plot to PDF

```{r dotplot_pdf,error=FALSE,message=FALSE}
#pdf("HvCa_AUC.pdf",height=4,width=6)
