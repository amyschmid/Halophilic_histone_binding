...
Measure logistic growth rates for each curve using grofit (see also helpful tutorial at https://barricklab.org/twiki/bin/view/Lab/ProtocolGrofit)
```{r}
#run grofit on each curve - grofit tries richards, gompertz, and logistic models on each curve
gro <- grofit (Grofittimeinput_tstop,Grofitdatainput_tstop)

#generate a summary table of fits
gro_table <- summary.gcFit(gro$gcFit)
write.table (gro_table, file = "grofit_table.txt", sep = "\t")
```
...
```
Visualise mu, lambda, A distribution for hstA v WT, first for logistic
```{r Boxplot}



m <- ggplot(Grofitoutput_tstop,aes(x=fct_rev(AddId),y=mu.model)) + 
  geom_jitter(width=0.2,aes(color=AddId)) + 
  stat_summary(fun=median,geom="crossbar",alpha=0.3,width=0.5,aes(color=AddId)) + 
  scale_color_manual(values=c("red","cadetblue3")) + 
  geom_signif(data=Grofitoutput_tstop,comparisons = list(c("Hv253","Hv28")),test="t.test",map_signif_level=TRUE) + 
  ggtitle("growth rate, logistic fit") + 
  theme_bw() + 
  xlab("Strain") + 
  ylab("mu") + 
  theme(axis.text=element_text(family="sans",size=12,face="bold"),axis.title=element_text(family="sans",size=14,face="bold"))

l <- ggplot(Grofitoutput_tstop,aes(x=fct_rev(AddId),y=lambda.model)) + 
  geom_jitter(width=0.2,aes(color=AddId)) + 
  stat_summary(fun=median,geom="crossbar",alpha=0.3,width=0.5,aes(color=AddId)) + 
  scale_color_manual(values=c("red","cadetblue3")) + 
  geom_signif(data=Grofitoutput_tstop,comparisons = list(c("Hv253","Hv28")),test="t.test",map_signif_level=TRUE) + 
  ggtitle("lag time, logistic fit") + 
  theme_bw() + 
  xlab("Strain") + 
  ylab("lambda") + 
  theme(axis.text=element_text(family="sans",size=12,face="bold"),axis.title=element_text(family="sans",size=14,face="bold"))

A <- ggplot(Grofitoutput_tstop,aes(x=fct_rev(AddId),y=A.model)) + 
  geom_jitter(width=0.2,aes(color=AddId)) + 
  stat_summary(fun=median,geom="crossbar",alpha=0.3,width=0.5,aes(color=AddId)) + 
  scale_color_manual(values=c("red","cadetblue3")) + 
  geom_signif(data=Grofitoutput_tstop,comparisons = list(c("Hv253","Hv28")),test="t.test",map_signif_level=TRUE) + 
  ggtitle("Carrying capacity, logistic fit") + 
  theme_bw() + 
  xlab("Strain") + 
  ylab("A") + 
  theme(axis.text=element_text(family="sans",size=12,face="bold"),axis.title=element_text(family="sans",size=14,face="bold"))

pdf("logistic-mu.pdf", height = 4, width = 6)
m
dev.off()

pdf("logistic-A.pdf", height = 4, width = 6)
A
dev.off()

pdf("logistic-lambda.pdf", height = 4, width = 6)
l
dev.off()
```
