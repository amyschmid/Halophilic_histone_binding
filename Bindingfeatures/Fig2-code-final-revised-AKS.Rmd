...
# Plotting for Figure 2, main manuscript
```{r plotting}
#set the colors
...
fig2b <- ggplot(Peakcharacteristics,aes(x=fct_inorder(Domain),y=Avg_width)) + 
...
fig2a <- ggplot(Peakcharacteristics,aes(x=fct_inorder(Domain),y=Percent_covered)) + geom_dotplot(alpha=0.7,binaxis="y",binwidth=1,stackdir="center",aes(fill=Specific_classification,color=Specific_classification)) + 
  stat_summary(fun=median,fun.min=median,fun.max=median,geom="crossbar",colour = "grey60", width=0.35, alpha = 0.5) + 
  theme_bw() + 
  ylim(c(0,25)) + 
  scale_fill_manual(values= my.colors) + 
...
pdf("fig2-pkfeatures-compiled-Sept2022.pdf", height = 11, width = 8)
